...
def launch_gui(accuracy, y_test, y_pred, X_test, label_encoders, rf, ch_data, original_data):
    ...
    def show_eda_report():
        report = ""
        report += "HEAD:\n" + str(ch_data.head()) + "\n\n"
        # info() prints to stdout, so capture it
        import io, sys
        buffer = io.StringIO()
        ...
        txt = scrolledtext.ScrolledText(eda_win, width=100, height=30)
    ...
    def show_model_eval():
        ...
        txt = scrolledtext.ScrolledText(eval_win, width=80, height=20)
        txt.pack(fill=tk.BOTH, expand=True)
    ...
    def make_prediction():
        ...
        for idx, col in enumerate(input_cols):
            tk.Label(form, text=col).grid(row=idx, column=0, sticky='w', padx=5, pady=5)
    ...
    # --- Buttons ---
    eda_btn = tk.Button(root, text="Show EDA Report", command=show_eda_report)
    eda_btn.pack(pady=10)

    eda_vis_btn = tk.Button(root, text="Show EDA Visualization", command=show_eda_visualization)
    eda_vis_btn.pack(pady=10)

    eval_btn = tk.Button(root, text="Show Model Evaluation Report", command=show_model_eval)
    eval_btn.pack(pady=10)

    pred_btn = tk.Button(root, text="Make Prediction", command=make_prediction)
